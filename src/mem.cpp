...
uint64_t Memory::getRegisterValue(Reg r) {
  ...
  auto it = std::find_if(std::begin(Registers),
                         std::end(Registers),
...
void Memory::dumpRegisters() {
  for(const auto& registerDescriptor: Registers) {
    spdlog::info("{:8} 0x{:016x}", registerDescriptor.name,
                 getRegisterValue(registerDescriptor.reg));
  }
}
...
void Memory::writeMemory(uint64_t address, uint64_t value) {
  ptrace(PTRACE_POKEDATA, pid, address, value);
}

uint64_t Memory::getPC() {
  return getRegisterValue(Reg::rip);
}

void Memory::setPC(uint64_t pc) {
  setRegisterValue(Reg::rip, pc);
}
